...
\title{Fast \& flexible exoplanet transit modeling using Bart}
...
parameters of the population of exoplanets. A necessary component of these
analyses is a tool for high performance probabilistic transit modeling. We
present a computationally efficient and extremely flexible algorithm called
\project{Bart} for accurately inferring and marginalizing over the physical
parameters of a planetary system conditioned on an observed light curve.
In particular, \project{Bart} uses a general non-parametric limb-darkening law
for the star. This model is much more flexible than the classic analytic
results from Madel \& Agol (2002) at minimal computational cost. As a
demonstration, we apply \project{Bart} to all N planets observed in the
\project{Kepler} optical photometric time series of the system Kepler-XXXXX
and the IR light curve of BLAH observed by \project{Spitzer}. Radial
velocity and astrometric observations of planetary systems can also be easily
